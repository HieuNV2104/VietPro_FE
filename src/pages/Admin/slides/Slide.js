...
const Slide = () => {
    ...
    const handleHideSlide = async (id) => {
        const confirm = window.confirm('Bạn có muốn ẩn ảnh này không ?');
        if (!confirm) return;

        try {
            const res = await hideSlide(id);
            if (res.status === 200) {
                if (page > 1) {
                    setSearchParams({ page: 1 });
                } else {
    ...
    const handleHShowSlide = async (id) => {
        const confirm = window.confirm('Bạn có muốn hiện ảnh này không ?');
        if (!confirm) return;

        try {
            const res = await showSlide(id);
            if (res.status === 200) {
                if (page > 1) {
                    setSearchParams({ page: 1 });
                } else {
            ...
            <div className="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
                <div className="row">
                    <ol className="breadcrumb">
                        ...
                        <li className="active">Danh sách ảnh Slide</li>
                    </ol>
                </div>
                <div className="row">
                    <div className="col-lg-12">
                        <h1 className="page-header">Danh sách ảnh Slide</h1>
                    </div>
                </div>
                <div id="toolbar" className="btn-group">
                    <Link
                        to={'/admin/ads/slides/create'}
                                            ...
                                            return (
                                                <tr key={slide?._id}>
                                                    ...
                                                    <td className="form-group">
                                                        ...
                                                        <Link
                                                            to={`/admin/ads/slides-${slide._id}/edit`}
